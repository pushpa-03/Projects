...
public class Game {
    Board board;
    Player[] players;
    boolean[] isAllowed; // Initially if  player get 1 on dice then he allowed to start the game
    int turn;
    Coordinates[] places;
    Dice dice;
    ...
    public void play() {
        while(true){
            ...
            //Initially checking for player get 1 on dice or not
            if(!isAllowed[turn]){
                if(num !=1){
                    turn = 1-turn;
                    continue;
    ...
    //It is only related to game class
